...
TEST_P(DurabilityWarmupTest, PendingSyncWrite) {
    ...
    // Check that the item is still pending - simplest way is to attempt another
    // write to the same key as this should fail as the previous one is still in
    // progress.
    setVBucketToActiveWithValidTopology();
    EXPECT_EQ(ENGINE_SYNC_WRITE_IN_PROGRESS, store->set(*item, cookie));
}
