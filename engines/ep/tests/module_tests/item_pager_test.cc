...
protected:
     ...
     */
    size_t populateUntilTmpFail(uint16_t vbid, rel_time_t expiry = 0) {
        size_t count = 0;
        ...
        for (result = ENGINE_SUCCESS; result == ENGINE_SUCCESS; count++) {
            ...
            auto item = make_item(vbid, key, value, ep_abs_time(expiry));
...
TEST_P(STItemPagerTest, ServerQuotaReached) {
    ...
    if (std::get<1>(GetParam()) == "fail_new_data") {
        ...
        EXPECT_EQ(count, engine->getVBucket(vbid)->getNumItems());
    } else {
        ...
        const auto numResidentItems =
                engine->getVBucket(vbid)->getNumItems() -
