...
| io_num_read               | Number of documents read from disk. Includes full and partial (meta-only) fetches         |
| io_num_write              | Number of documents written to disk                                                       |
| io_document_read_bytes    | Number of bytes read while fetching documents (key + value + rev_meta)                  |
| io_document_write_bytes   | Number of bytes written while flushing documents (key + value + rev_meta                 |
